/*
  ...
  1. Tables to create (if not exists)
    - All tables already exist based on schema, so we'll use IF NOT EXISTS
...
-- Create user_donations table (if not exists)
...
-- Create donation_certificates table (if not exists)
...
-- Create user_chats table (if not exists)
...
DO $$
BEGIN
  -- Drop and recreate app_users policies
  ...
  -- Drop and recreate user_donations policies
  ...
  -- Drop and recreate donation_certificates policies
  ...
  -- Drop and recreate user_chats policies
