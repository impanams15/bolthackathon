/*
  # Create additional tables for donation system

  1. New Tables
    - `users` - User profiles with Algorand addresses
    - `donations` - Donation transaction records  
    - `certificates` - ASA certificate records
...
-- Create donations table (different from existing donations table)
...
-- Create chats table (different from existing chat_conversations table)
...
BEGIN
  ...
  -- Drop policies for donations table
  ...
  -- Drop policies for certificates table
